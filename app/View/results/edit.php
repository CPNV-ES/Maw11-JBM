...
$labelTitle = $exercise['title']; ?>
    <h1>Your take</h1>
    <p class="lead">Bookmark this page, it's yours. You'll be able to come back later to finish</p>
        ...
        <?php foreach ($exercise['fields'] as $field): ?>
            ...
            <?php if ($fulfillment['fields_id'] == $field['id']): ?>
                <div class="field mb-3">
                    ...
                    <input class="form-control"  value="<?=$fulfillment['answer'] ?>" type="text" name="answer_<?=$fulfillment['id']?>" id="answer_<?=$field['id']?>">
                </div>
                <?php endif; ?>
                <?php endforeach; ?>
        <?php endforeach; ?>
